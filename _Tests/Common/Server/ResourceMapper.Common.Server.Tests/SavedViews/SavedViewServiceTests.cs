...
namespace ResourceMapper.Common.Server.Tests.SavedViews
{
    ...
    public class SavedViewServiceTests
    {
        ...
        [Theory]
        [InlineData("no-leading-question-mark")]
        [InlineData("?has whitespace")]
        [InlineData("?has#fragment")]
        [InlineData("javascript:alert(1)")]
        [InlineData("//evil.example.com")]
        public async Task SaveAsync_QueryStringIsNotAPlainQuery_ReturnsValidation(string queryString)
        {
            var request = new SaveViewRequest { Name = "View A", QueryString = queryString };

            var response = await _sut.SaveAsync("steve", request, CancellationToken.None);

            response.IsSuccess().Should().BeFalse(
                "because a saved query is replayed by appending it to a path, so anything that is not "
                + "a plain query string could navigate somewhere else entirely");
        }

        [Fact]
        public async Task SaveAsync_QueryStringOverTheCap_ReturnsValidation()
        {
            var request = new SaveViewRequest
            {
                Name = "View A",
                QueryString = "?n=100&f=" + new string('x', SavedViewValidation.MaxQueryStringLength)
            };

            var response = await _sut.SaveAsync("steve", request, CancellationToken.None);

            response.IsSuccess().Should().BeFalse(
                "because the column is NVARCHAR(MAX) but an endpoint with no limit would accept megabytes");
        }

        [Theory]
        [InlineData((char)10)]   // newline
        [InlineData((char)9)]    // tab
        [InlineData((char)0)]    // null
        [InlineData((char)27)]   // escape
        public async Task SaveAsync_NameContainsControlCharacters_ReturnsValidation(char control)
        {
            var name = "Has" + control + "Control";
            var response = await _sut.SaveAsync("steve", Req(name), CancellationToken.None);

            response.IsSuccess().Should().BeFalse(
                "because a name with control characters corrupts the menu it is rendered into");
        }

        [Theory]
        [InlineData("sv-1; DROP TABLE SavedView--")]
        [InlineData("sv 1")]
        [InlineData("sv-<script>")]
        [InlineData("../../etc/passwd")]
        public async Task SaveAsync_UidIsNotAPlainToken_ReturnsValidationAndDoesNotCallRepo(string uid)
        {
            var request = new SaveViewRequest { SavedViewUid = uid, Name = "View A", QueryString = "?n=100" };

            var response = await _sut.SaveAsync("steve", request, CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because uids are generated by this service, not by callers");
            _repo.Verify(r => r.UpsertAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(),
                    It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never,
                "because a malformed uid never reaches the database, parameterised or not");
        }

        [Fact]
        public async Task SaveAsync_UidLongerThanTheColumn_ReturnsValidation()
        {
            var request = new SaveViewRequest
            {
                SavedViewUid = new string('a', SavedViewValidation.MaxUidLength + 1),
                Name = "View A",
                QueryString = "?n=100"
            };
            ...
            response.IsSuccess().Should().BeFalse("because VARCHAR(40) would silently truncate it");
        }

        [Fact]
        public async Task SaveAsync_RepositoryReportsDuplicate_ReturnsValidationNotAnError()
        {
            _repo.Setup(r => r.UpsertAsync("steve", It.IsAny<string>(), "Taken", It.IsAny<string>(),
                    It.IsAny<CancellationToken>()))
                 .ReturnsAsync(("duplicate", ""));

            var response = await _sut.SaveAsync("steve", Req("Taken"), CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because names are unique per owner");
            _repo.Verify(r => r.UpsertAsync("steve", It.IsAny<string>(), "Taken", It.IsAny<string>(),
                    It.IsAny<CancellationToken>()), Times.Once,
                "because the collision is detected inside the procedure, not by a check-then-write that "
                + "two concurrent saves could both pass");
        }

        [Fact]
        public async Task ReorderAsync_RepeatedUid_ReturnsValidation()
        {
            var response = await _sut.ReorderAsync("steve",
                new List<string> { "sv-a", "sv-a" }, CancellationToken.None);

            response.IsSuccess().Should().BeFalse(
                "because a repeated uid means two positions for one view, which is incoherent");
        }

        [Fact]
        public async Task ReorderAsync_MalformedUid_ReturnsValidationAndDoesNotCallRepo()
        {
            var response = await _sut.ReorderAsync("steve",
                new List<string> { "sv-a", "'; DELETE FROM SavedView--" }, CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because the list arrives straight from an HTTP body");
            _repo.Verify(r => r.ReorderAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(),
                    It.IsAny<CancellationToken>()), Times.Never, "because nothing malformed reaches the TVP");
        }

        [Fact]
        public async Task ReorderAsync_AbsurdlyLongList_ReturnsValidation()
        {
            var many = Enumerable.Range(0, 501).Select(i => "sv-" + i).ToList();

            var response = await _sut.ReorderAsync("steve", many, CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because one owner's list is never that long");
        }

        [Fact]
        public async Task DeleteAsync_MalformedUid_ReturnsValidation()
        {
            var response = await _sut.DeleteAsync("steve", "sv-1 OR 1=1", CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because uids are plain tokens");
        }

        [Fact]
        public async Task SaveAsync_OwnerIdLongerThanTheColumn_ReturnsValidation()
        {
            var owner = new string('o', SavedViewValidation.MaxOwnerIdLength + 1);

            var response = await _sut.SaveAsync(owner, Req("View A"), CancellationToken.None);

            response.IsSuccess().Should().BeFalse("because VARCHAR(64) would truncate it into another owner");
        }
