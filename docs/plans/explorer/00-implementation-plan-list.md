...
| 7b | **Share** | **Share link** `/explore/shared/{ShareId}` → **read-only** load when `clientId` ≠ owner (suppress owner `DiagramUid`); **"Save a copy to mine"**; recovery-key restore; guard `loadJson` against malformed persisted JSON (slice-7a review note). | 7a | **Next** |
