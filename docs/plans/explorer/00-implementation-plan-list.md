...
| 6 | **[Client identity (localStorage)](./06-client-identity.md)** | `clientId` GUID **get-or-create in `localStorage` via JS interop** (deferred past prerender; matches `Home.razor`), exposed as a small reusable helper the explorer page uses; optional "copy recovery key". **Not** a cookie/middleware primitive (see design §7.1). | 2 | **Next** |
