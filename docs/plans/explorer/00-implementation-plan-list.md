...
| 8 | **[Export](./08-export.md)** | **Copy PNG to clipboard**, **Save PNG** (`cy.png`), **Save SVG** (`cytoscape-svg`), and browser **Print → PDF**. Plus **copy share link as a rich hyperlink** (clipboard `text/html` `<a href>` with the **diagram name** as anchor text, so pasting into Teams/Outlook shows "My Diagram", not the raw URL). | 2 | **Next** |
