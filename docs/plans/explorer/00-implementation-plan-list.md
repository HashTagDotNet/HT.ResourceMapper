...
| 2 | **Anonymous client identity** | `rm_client_id` cookie ensured by middleware (durable GUID); `IClientIdentityAccessor` (reads/creates via `IHttpContextAccessor`); registration in `Program.cs`; unit tests for accessor logic. Reusable primitive. No UI. | — | **Next** |
