...
| 7a | **[Persist: Save/Open/Delete](./07a-persist-crud-ui.md)** | Toolbar (Save / Save-As / Open-Recent + per-row Delete, absorbing Fit / Collapse-all / Display / client caption); serialize canvas ⇄ **self-contained `DiagramJson`**; owner CRUD via `IDiagramService` + `clientId`. | 4,5,6 | **Next** |
| 7b | **Share** | **Share link** `/explore/shared/{ShareId}` → **read-only** load when `clientId` ≠ owner (suppress owner `DiagramUid`); **"Save a copy to mine"**; recovery-key restore. | 7a | Planned |
