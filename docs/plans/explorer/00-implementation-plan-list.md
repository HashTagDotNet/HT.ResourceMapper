...
| 10 | **[Client settings migration](./10-client-settings-migration.md)** *(last)* | `ClientSetting` table (`ClientId`, `SettingKey`, `SettingJson`) + sprocs + `IClientSettingsService`, reusing slice-6 `clientId`; migrate the **home grid filters/view** off ad-hoc `localStorage` (`Home.razor`) to the server. | 5,6 | **Next** |
