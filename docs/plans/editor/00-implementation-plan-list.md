...
| 5 | **[Import wiring — Domain](./05-import-wiring-domain.md)** | **Domain now enters identity**: `Resource_Upsert`/`Resource_GetAllKeys` gain the domain-tag join → full `(domain+type+key)` triple (`Resource_Upsert` owns/writes the domain tag); `import.defaults.domain` + per-resource override; idempotency on the triple; `ResourceTag_SetForResource` domain-safety; wire `dependencies`→`DependsOn` (resolve by `(domain+key)`); validation updates (type required, content-type→Text/Link, domain vocab, triple dedup). | 2–4 | Planning |
