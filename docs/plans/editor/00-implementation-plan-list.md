...
| 6 | **Editor/Details UI — core** | Tabbed component; Create (wizard) / Edit (property-sheet) / View (read-only + Edit) modes; **General** tab (type/domain/name/auto-key/description, identity preview, early uniqueness); **Review** tab + validation gate; save + success toast; routes `/resources`, `/resources/<uid>`; "Create Resource" in hamburger. | 5 | Planning |
