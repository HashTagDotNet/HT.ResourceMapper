...
| 3 | **Contracts & DTOs** | Fill empty `ResourceDto`/`ResourceTagDto`/`TagDefinitionDto`/`TagContentTypeDto`; editor DTOs (expanded `ResourceEditorModel`, tag rows, dependency rows, identity preview); relationship + facet DTOs. | 2 | Planning |
