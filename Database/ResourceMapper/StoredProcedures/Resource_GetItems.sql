...
BEGIN
    ...
    BEGIN
        SET @whereClause = '
        WHERE (
            r.ResourceKey LIKE @SafeSearchParam ESCAPE '']''
            OR r.ResourceName LIKE @SafeSearchParam ESCAPE '']''
            OR r.Description LIKE @SafeSearchParam ESCAPE '']''
            OR EXISTS (
                ...
                AND (rt2.TagValue LIKE @SafeSearchParam ESCAPE '']'' OR td2.TagDefinitionKey LIKE @SafeSearchParam ESCAPE '']'')
            )
        )';
    END
    ...
    BEGIN
        EXEC sp_executesql @sql, 
            N'@SafeSearchParam NVARCHAR(255), @TotalRecords INT OUTPUT', 
    ...
    TagsWithPriority AS (
        SELECT 
            ...
            rt.TagValue,
            CASE 
                WHEN @SafeSearchParam IS NOT NULL AND (
                    rt.TagValue LIKE @SafeSearchParam ESCAPE '']''
                    OR td.TagDefinitionKey LIKE @SafeSearchParam ESCAPE '']'')
    ...
    BEGIN
        EXEC sp_executesql @sql, 
            N'@SafeSearchParam NVARCHAR(255), @Skip INT, @Take INT', 
